...
const SUMMARY_PROMPT = (videoTexts) => `Act as a professional media content analyst. Please analyze the provided list of video titles and summarize the blogger's niche and content.
...
async function main() {
    ...
    // 并发重新生成
    const summaries = await mapWithConcurrency(truncated, async (row, i) => {
    ...
    const fullMap = new Map(fullRes.data.map((r) => [String(r.id), r]));
