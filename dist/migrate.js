...
// ── AI 摘要生成 ──────────────────────────────────────────────────────────────
async function generateSummary(videoTexts) {
...
// ── 自动创建目标集合（若不存在）────────────────────────────────────────────────
async function ensureTargetCollection(vectorDim) {
...
async function main() {
    ...
    const vectorField = desc.schema.fields.find((f) => f.data_type === 'FloatVector' || f.dataType === 101);
    const vectorDim = Number(vectorField?.type_params?.find((p) => p.key === 'dim')?.value ?? 1536);
    // 确保目标集合存在
    ...
    while (true) {
        if (totalInserted >= MIGRATE_LIMIT)
            break;
        // 1. 拉取数据
        ...
        let newRows = rows.filter((r) => {
            if (existingIds.has(String(r.id)))
                return false;
            if (!String(r.videoTexts ?? '').trim())
                return false;
            return true;
        });
        // 截断到剩余配额
        const remaining = MIGRATE_LIMIT - totalInserted;
        if (newRows.length > remaining)
            newRows = newRows.slice(0, remaining);
        ...
        if (newRows.length > 0) {
            // 2. 并发生成 ai_summary
            const summaries = await mapWithConcurrency(newRows, async (row, i) => {
