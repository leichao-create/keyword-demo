...
async function ensureV2Collection(vectorDim) {
...
async function main() {
    ...
    const vectorField = desc.schema.fields.find((f) => f.data_type === 'FloatVector' || f.data_type === 101);
    const vectorDim = Number(vectorField?.type_params?.find((p) => p.key === 'dim')?.value ?? 3072);
    ...
    while (true) {
        if (totalInserted >= MIGRATE_LIMIT)
            break;
        // 1. 分页读取 v1
        const queryRes = await withRetry(() => source.query({
        ...
        if (newRows.length > 0) {
            ...
            const rowCount = stats.stats.find((s) => s.key === 'row_count')?.value ?? '?';
