...
res.data.forEach((row, i) => {
    ...
    console.log(`[${String(i + 1).padStart(4)}] ${String(row.nickname ?? '').padEnd(24)} | ${String(row.platform ?? '').padEnd(9)} | ${String(row.region ?? '').padEnd(4)} | 粉丝:${String(row.followerCount ?? 0).padStart(8)} | ${summary}`);
});
